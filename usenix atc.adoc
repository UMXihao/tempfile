...
对于Transformer架构的模型，通用的模型加载方法就是按Transformer block层加载footnote:[1]，如果需要卸载运算只需要指定层数到CPU和GPU进行运算。针对GPU集群场景还存在一种模型加载方法，先将模型按层加载进内存，然后层内张量按行分割模型到多个GPU中参与运算footnote:[2]，最后由主GPU完成汇聚运算。
